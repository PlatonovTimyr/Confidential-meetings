// ===== Confidential Meetings - SCREEN SHARE FIX =====
...
function setupCreate() {
    ...
    $('#createMeetingBtn').onclick = async () => {
        ...
        try {
            ...
            await captureMedia();
            const cam=$('#createCameraToggle').checked; const mic=$('#createMicToggle').checked;
...
function setupJoin() {
    ...
    $('#connectByLinkBtn').onclick = async () => {
        const link = $('#meetingLinkInput').value.trim(); if(!link)return alert('Вставьте ссылку');
        stopPreview();
        try {
            ...
            switchScreen('meetingScreen'); updateUI(); updateHostUI(); startTimer(); showNotification('🔄 Подключение...');
            await captureMedia();
...
function setupInvite() {
    ...
    $('#inviteConnectBtn').onclick = async () => {
        ...
        try { stopPreview(); switchScreen('meetingScreen'); updateUI(); updateHostUI(); startTimer(); showNotification('🔄 Подключение...'); await captureMedia(); const cam=$('#inviteCameraToggle').checked; const mic=$('#inviteMicToggle').checked; if(!cam&&AppState.localStream){const t=AppState.localStream.getVideoTracks()[0];if(t){t.enabled=false;AppState.cameraEnabled=false;}} if(!mic&&AppState.localStream){const t=AppState.localStream.getAudioTracks()[0];if(t){t.enabled=false;AppState.micEnabled=false;}} initTrystero(); } catch(e) { alert('Ошибка: '+e.message); }
    };
    $('#changeInviteAvatar').onclick = () => { const e=getEmoji(); $('#inviteAvatarEmoji').textContent=e; AppState.userEmoji=e; };
}

function updateHostUI() { const btn=$('#shareBtn'); if(btn)btn.style.display=AppState.isHost?'flex':'none'; }
...
async function captureMedia() {
    try { AppState.localStream=await navigator.mediaDevices.getUserMedia({video:{width:1280,height:720},audio:true}); const lv=$('#localVideo');if(lv){lv.srcObject=AppState.localStream;lv.parentElement.classList.remove('hidden');} $('#localAvatarWrapper').classList.add('hidden'); updateMicBtn(); updateCamBtn(); } catch(e) { AppState.cameraEnabled=false; $('#localVideo').parentElement.classList.add('hidden'); $('#localAvatarWrapper').classList.remove('hidden'); }
}
    ...
    try {
        ...
        for (const [peerId, peer] of AppState.peers) {
            try {
                ...
                console.log('📺 Экран отправлен пиру:', peerId);
            } catch(e) {
                console.error('Ошибка отправки экрана:', peerId, e);
            }
        }
...
// ===== Управление =====
function toggleMic() { if(!AppState.localStream)return; const t=AppState.localStream.getAudioTracks()[0]; if(t){t.enabled=!t.enabled;AppState.micEnabled=t.enabled;updateMicBtn();showNotification(AppState.micEnabled?'🎤 Микрофон включен':'🔇 Микрофон выключен');} }
...
function toggleCamera() { if(!AppState.localStream)return; const t=AppState.localStream.getVideoTracks()[0]; if(t){t.enabled=!t.enabled;AppState.cameraEnabled=t.enabled;updateCamBtn();showNotification(AppState.cameraEnabled?'📹 Камера включена':'📷 Камера выключена');if(AppState.cameraEnabled){$('#localVideo').parentElement.classList.remove('hidden');$('#localAvatarWrapper').classList.add('hidden');}else{$('#localVideo').parentElement.classList.add('hidden');$('#localAvatarWrapper').classList.remove('hidden');}sendMyInfo();} }
function updateCamBtn() { const b=$('#cameraBtn'); if(AppState.cameraEnabled)b.classList.remove('off'); else b.classList.add('off'); }

function sendChat() { const ci=$('#chatInput');if(!ci)return;const t=ci.value.trim();if(!t)return;showChatMsg(AppState.userName,t,true);if(AppState.sendChatMsg)AppState.sendChatMsg({sender:AppState.userName,text:t,timestamp:Date.now()});ci.value='';ci.focus(); }
function showChatMsg(sender,text,isMine) { const cm=$('#chatMessages');if(!cm)return;const d=document.createElement('div');d.className='chat-message '+(isMine?'mine':'other');d.innerHTML='<span class="sender">'+sender+'</span><span class="text">'+text.replace(/</g,'&lt;')+'</span>';const empty=cm.querySelector('.chat-empty');if(empty)empty.remove();cm.appendChild(d);cm.scrollTop=cm.scrollHeight; }

async function startScanner() { try{const s=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});AppState.scannerStream=s;$('#scannerVideo').srcObject=s;const iv=setInterval(()=>{if(!AppState.scannerStream){clearInterval(iv);return;}const v=$('#scannerVideo');if(!v||v.readyState<2)return;const c=document.createElement('canvas');c.width=v.videoWidth;c.height=v.videoHeight;const ctx=c.getContext('2d');ctx.drawImage(v,0,0);const code=jsQR(ctx.getImageData(0,0,c.width,c.height).data,c.width,c.height);if(code&&code.data){clearInterval(iv);stopScanner();try{const u=new URL(code.data);const h=u.hash.slice(1);const p=new URLSearchParams(h);const r=p.get('room');if(!r)return;AppState.roomId=r;AppState.isHost=false;AppState.cameraEnabled=true;AppState.micEnabled=true;AppState.userName=$('#joinUserName').value.trim()||'Гость';switchScreen('meetingScreen');updateUI();updateHostUI();startTimer();showNotification('🔄 Подключение...');captureMedia().then(()=>{const cam=$('#joinCameraToggle').checked;const mic=$('#joinMicToggle').checked;if(!cam&&AppState.localStream){const t=AppState.localStream.getVideoTracks()[0];if(t){t.enabled=false;AppState.cameraEnabled=false;}}if(!mic&&AppState.localStream){const t=AppState.localStream.getAudioTracks()[0];if(t){t.enabled=false;AppState.micEnabled=false;}}initTrystero();});}catch(e){alert('Неверный QR-код');}}},100);}catch(e){alert('Ошибка камеры');switchScreen('joinScreen');} }
...
function checkUrl() { const h=window.location.hash;if(!h||h==='#')return false;const hc=h.startsWith('#')?h.slice(1):h;const p=new URLSearchParams(hc);const r=p.get('room');if(!r)return false;AppState.roomId=r;AppState.isHost=false;$$('.screen').forEach(s=>s.classList.add('hidden'));$('#inviteScreen').classList.remove('hidden');setTimeout(()=>{try{new QRCode($('#inviteQRCanvas'),{text:location.href,width:200,height:200});}catch(e){}},500);setTimeout(()=>{if($('#inviteCameraToggle').checked)startPreview($('#invitePreviewVideo'),$('#inviteAvatarEmoji'));},800);return true; }

function updateUI() { const role=AppState.isHost?'Организатор':'Участник';$('#localName').textContent=(AppState.userName||'Вы')+' • '+role;$('#localAvatarEmoji').textContent=AppState.userEmoji;if(AppState.cameraEnabled){$('#localVideo').parentElement.classList.remove('hidden');$('#localAvatarWrapper').classList.add('hidden');}else{$('#localVideo').parentElement.classList.add('hidden');$('#localAvatarWrapper').classList.remove('hidden');}if(AppState.isHost)$('#hostChip').classList.remove('hidden');else $('#hostChip').classList.add('hidden');updateMicBtn();updateCamBtn(); }
function updateRemote(info) { if(!info||!info.name)return;const role=info.role||'';$('#remoteName').textContent=info.name+(role?' • '+role:'');const re=document.querySelector('#remoteAvatarWrapper .avatar-emoji');if(re&&info.emoji)re.textContent=info.emoji;$('#remoteCard').classList.remove('hidden');$('#emptyState').classList.add('hidden');if(!info.hasVideo){$('#remoteVideo').parentElement.classList.add('hidden');$('#remoteAvatarWrapper').classList.remove('hidden');}else{$('#remoteVideo').parentElement.classList.remove('hidden');$('#remoteAvatarWrapper').classList.add('hidden');}updateCount(); }
function updateCount() { $('#participantCount').textContent=AppState.peers.size+1; }

function startTimer() { AppState.startTime=Date.now();if(AppState.timerInterval)clearInterval(AppState.timerInterval);AppState.timerInterval=setInterval(()=>{const e=Math.floor((Date.now()-AppState.startTime)/1000);const m=Math.floor(e/60).toString().padStart(2,'0');const s=(e%60).toString().padStart(2,'0');$('#timerDisplay').textContent=m+':'+s;},1000); }
