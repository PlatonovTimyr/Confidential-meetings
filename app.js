...
function setupCreate() {
    ...
    $('#createMeetingBtn').onclick = async () => {
        ...
        try {
            ...
            initBugout(); switchScreen('meetingScreen'); updateUI(); updateHostUI(); startTimer();
...
function setupInvite() {
    ...
    $('#inviteConnectBtn').onclick = async () => {
        ...
        try { stopPreview(); switchScreen('meetingScreen'); updateUI(); updateHostUI(); startTimer(); showNotification('🔄 Подключение...'); await captureMedia(); const cam=$('#inviteCameraToggle').checked; const mic=$('#inviteMicToggle').checked; if(!cam&&AppState.localStream){const t=AppState.localStream.getVideoTracks()[0];if(t){t.enabled=false;AppState.cameraEnabled=false;}} if(!mic&&AppState.localStream){const t=AppState.localStream.getAudioTracks()[0];if(t){t.enabled=false;AppState.micEnabled=false;}} initBugout(); } catch(e) { alert('Ошибка: '+e.message); }
    };
...
function setupMeeting() {
    ...
    $('#changeLocalAvatar').onclick = () => { const e=getEmoji(); $('#localAvatarEmoji').textContent=e; AppState.userEmoji=e; };
}
...
function sendChat() { const ci=$('#chatInput');if(!ci)return;const t=ci.value.trim();if(!t)return;showChatMsg(AppState.userName,t,true);if(AppState.bugout)AppState.bugout.send(JSON.stringify({type:'chat',sender:AppState.userName,text:t}));ci.value='';ci.focus(); }
...
function updateRemote(info) { if(!info||!info.name)return;const role=info.role||'';$('#remoteName').textContent=info.name+(role?' • '+role:'');updateCount(); }
...
function hangUp() { 
    ...
    if(AppState.bugout){try{AppState.bugout.close();}catch(e){}AppState.bugout=null;}
